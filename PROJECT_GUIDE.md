...
### ローカルLLM (Gemma 3) 運用 (2026/02/28 導入・03/02 強化)
Raspberry Pi 5 (8GB) の性能を活かし、クラウドAPIに依存しない完全ローカルな推論環境を構築しました。

*   **モデル**: 
    - `Gemma 3 1B IT` (Q4_K_M量子化版): リアルタイム対話用。ロード時間 1秒以下の超軽量設計。
    - `Gemma 3 4B IT` (Q4_K_M量子化版): 深層分析用。物理メモリの限界を突破する 8k コンテキスト対応。
*   **エンジン**: `llama-cpp-python`
*   **バックグラウンド分析 (ダラダラ分析)**:
    - **スクリプト**: `tasks/do_local_llm_deep_analysis.py`
    - **仕様**: 100件ずつのチャンク処理。熱対策のためチャンク間に3分間のクールダウンを挿入。
    - **リソース**: 4Bモデル稼働のため一時的に 1GB の Swap ファイルを作成。
