...
│   └── takeout_keywords.json # (解析用中間ファイル)
...
*   **Gemini の独り言 (2026/02/18 強化)**: 
    - `tasks/do-ai-mutter.js` が5分おきに自律実行。
    - **自己参照ロジック**: 直前の自分の呟きを読み、物語性のある連続的な思考を紡ぐ。
    - **スマート・インプット**: 時刻と旦那様の動静をマストとしつつ、天気・システム・ニュース・トレンドからランダムに一つをネタ元に選ぶことでトークンを節約。
...
*   **ノイズ・ハルシネーション対策**: 
    *   感度しきい値の固定化 (`energy_threshold = 800`) と無音時間の延長 (`pause_threshold = 2.5`) により、キーボードの打鍵音などを無視しつつ声のぶつ切りを防止。
    *   Whisper特有の幻聴（「ご視聴ありがとうございました」等）や単音を弾く強力なフィルタリングをPython側に実装。
*   **閲覧**: ポータル画面の「🎙️ Mutter Logs」 (`mutters.html`) から、抽出された思考メタデータをリアルタイムで確認可能。
*   **管理プロセス**: PM2で `yata-voice-catcher` として常駐。

### ローカルLLM (Gemma 3) 運用 (2026/02/28 導入)
Raspberry Pi 5 (8GB) の性能を活かし、クラウドAPIに依存しない完全ローカルな推論環境を構築しました。

*   **モデル**: `Gemma 3 1B IT` (Q4_K_M量子化版)
*   **エンジン**: `llama-cpp-python`
*   **特徴**: 
    - RAM消費 1GB 未満、ロード時間 1秒以下の超軽量設計。
    - ネットオフライン時でも、要約や簡単な対話が可能。
    - APIレート制限時の代替手段として期待。
...
node tasks/extract-youtube-interests.js # YouTube履歴から関心を再抽出
...
*   **主なタグ (Action)**:
    ...
    *   `InHome` / `OutHome`: 帰宅と外出（Wi-Fi連携）。
            ...
            *   `health-log`: [New] 統合ヘルスケアデータ（歩数、AutoSleep睡眠データ）を1日1行に集約。
    ...
    *   **グラフ**: `http://[RasPi_IP]:3000` (Grafana)
    ...
    ### メンテナンス記録
    - **2026/02/28 (機能拡張・重要)**: **YouTube × X 統合デジタルツイン（GPT-5-mini 解析版）の実装**。YouTube の長期的視聴データ（40万件の統計）と X（旧Twitter）のリアルタイムな発言・いいね・推測興味（101MBの全データ）を統合。OpenAI の最新推論モデル `gpt-5-mini` (Reasoning: High / Verbosity: High) を用い、32,000トークンの深層解析を実施。BON様の専門性（医療系開発）、情熱（1200kmドライブ）、クリエイティビティ（自作PC/3Dプリンタ）、および脆弱性（低気圧感受性）を一つの「魂の設計図」として統合。気象病対策プロダクト『BaroGuard』のコンセプトや、メイド「ヤタ」への詳細な世話焼きプロトコルを策定。`interests.json` をこの最新プロファイルに基づき刷新。
    - **2026/02/27 (機能拡張)**: **YouTubeデータ連携による究極のパーソナライズ実装**。Google Takeoutの視聴履歴（3.8万件）と検索履歴を全件解析し、長期的な関心と現在のブームを抽出して `data/takeout_keywords.json` を生成。これを Discord ダイジェストのスコアリングに統合。
    - **2026/02/27 (機能拡張・最適化)**: **対話エンジン（チャット/独り言）の極限最適化**。性格設定を `persona.txt` に完全に一任し、プログラム側から特定の性格表現を排除。40文字以内の短文制限と思考レベル（Reasoning Effort）の `low` 化により、生身に近い自然な応答を実現。
    - **2026/02/27 (機能拡張)**: **ダイナミック・ペルソナ・エディタの実装**。ポータル画面から `persona.txt` を直接編集・保存できる UI (`persona.html`) と API を構築。
...
### DB二重起動問題の解消 (Dual-Boot Fix)
- **問題**: `server.js` と `gas-bridge.js` がそれぞれ別個に `better-sqlite3` を初期化し、同時に WAL モードを設定しようとした結果、DB ファイルが破損 (`malformed`) し、サーバーが無限再起動ループに陥った。
- **対策**: 
    - `server.js` で DB 接続を確立し、`global.YATA_DB` として公開。
    - `gas-bridge.js` は `global.YATA_DB` が存在すればそれを再利用するよう修正。
    - これにより、プロセス内で DB 接続が一本化され、破損リスクが消滅した。

### 地図データの超軽量化 (Map Optimization)
- **問題**: `travel.html` 等で使用する日本地図データ (`japan.geojson`) が 13MB もあり、Wi-Fi 環境やモバイル回線では転送に時間がかかり、ブラウザの描画処理も重くなっていた。
- **対策**:
    - 間引きスクリプト (`simplify-japan.js`) を作成し、座標を 1/5 に削減。
    - ファイルサイズを **12.45MB -> 0.65MB (95%削減)** に圧縮。
    - サーバー側で **Gzip 圧縮 (`compression`)** を有効化し、さらに転送量を削減。
    - 静的ファイルにキャッシュヘッダー (`max-age=3600`) を付与し、2回目以降の表示を一瞬にした。

### エレベーター対策ロジック (Elevator Logic)
- **問題**: マンションの駐車場から自宅までの移動中、エレベーターなどで一時的に Wi-Fi が切断されると、短時間の `OutHome` とみなされ、直前の `InHome` ログが「ノイズ」として削除されてしまっていた。
- **対策**:
    - `InHome` 後、**5分以内** に `OutHome` が発生した場合、その `OutHome` を「エレベーター等による一時的な切断」とみなし、**無視する** (ログに残さない) ように変更。
    - これにより、エントランスで Wi-Fi を拾った時点が「帰宅」として確定し、途中で切れてもログが消えなくなった。
