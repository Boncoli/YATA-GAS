...
## 運行・プロセス・マップ (Service Architecture)

システムが「どこで何をしているか」の全体像です。

| プロセス名 | 実行主体 | 主要な責務 | 参照/更新テーブル | ポート/トリガー |
| :--- | :--- | :--- | :--- | :--- |
| **YATA Server** | `server.js` (PM2) | Web UI提供、iPhoneログ受信、API提供 | 全テーブル | **3001** |
| **YATA Bot** | `discord-bot.js` (PM2) | Discord対話、メンション返答 | `ai_chat_log` | Discord Mention |
| **Voice Catcher** | `catcher.py` (PM2) | マイク音声の常時監視・テキスト化 | `mutter_logs` | (現在停止中) |
| **Local Mutter** | `do-local-mutter.py` | 1時間毎の時報ボヤキ (Gemma 3 4B) | `collect`, `drive_logs` | **Cron (0分)** |
| **Daily Digest** | `do-discord-digest.js` | 毎朝のニュース速報 (Gemini API) | `collect` | **Cron (10:02)** |
| **Main Task** | `yata-task.js` | RSS収集、AI要約、DBメンテ | `collect`, `trends` | Cron (26,56分) |
...
4.  **常駐ボット (`modules/discord-bot.js`)**: リアルタイム対話と自律的な独り言を担当。
...
### メンテナンス記録
- **2026/03/05 (機能拡張・安定化)**: **YATA.js最新化とヘルスケア統合の堅牢化**。
...
- **2026/03/01 (LLM運用革命)**: **Gemma 3 4B-IT による100件一括深層解析の成功**。Raspberry Pi 5 において 1GB Swap 作成により物理メモリ限界を突破し、長時間バッチ解析を実現。
- **2026/03/03 (メンテナンス)**: **ヘルスケア受信ロジックの堅牢化**。SQLiteの名前付きパラメータを導入し、iPhoneショートカットからのデータ同期エラーを根絶。
- **2026/03/03 (機能拡張)**: **ローカルLLM（Gemma 3 4B）による自律的な「時報ボヤキ」の実装**。
- **2026/03/01 (メンテナンス)**: **DB構造の整理とヘルスケアデータの完全統合**。`health_logs` テーブルを削除し、`daily_health` へ一本化。
- **2026/02/28 (機能拡張・重要)**: **YouTube × X 統合デジタルツイン（GPT-5-mini 解析版）の実装**。
- **2026/02/28 (機能拡張)**: **「完全版・高濃度アーカイブ (41.24MB)」の生成と物理保護の実装**。アーカイブファイルを `chmod a-w` で読み取り専用に設定。
- **2026/02/27 (機能拡張)**: **YouTubeデータ連携による究極のパーソナライズ実装**。`data/takeout_keywords.json` を生成し Discord ダイジェストに統合。

---

## 3. システム安定化・高速化の記録 (2026/02/25)
