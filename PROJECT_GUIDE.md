...
*   **Discord 投稿 (パーソナライズド・ハイライト)**: `tasks/do-discord-digest.js` により、毎朝 10:02 に前日のハイライトを投稿します。
    *   **進化ポイント (2026/02)**: 投稿直前に「ユーザーが詳細解析（クリック）した記事の履歴」を GPT-5 Mini で分析し、`interests.json` を動的に更新。そのプロファイルを元に AI が「ユーザーが特に関心を持ちそうなニュース」を優先的にピックアップし、解説を加える「パーソナライズ機能」を搭載。
    *   **履歴保存**: 投稿内容は `history` テーブルに `DISCORD_DIGEST` というキーワードで保存されます。
...
1.  **起動 (`@reboot`)**: システム起動時に `run-ram.sh` が自動実行され、SDカードからRAMへDBを展開。これにより常駐プロセス (`server.js`) の不整合を防止。
2.  **収集 (`do-collect`)**: RSSフェッチ -> 重複チェック -> RAM DBへ保存。
3.  **加工 (`do-summarize`)**: 未要約記事抽出 -> LLM要約・ベクトル化 -> RAM DB更新。
4.  **配信**: 
    - Discord: `do-discord-digest.js` が昨日のまとめを生成・投稿。
    - メール: `do-send-report.js` が週刊総集編を生成して送信。
5.  **終了・同期**: `run-ram.sh` が終了の度に RAM DB (本体+WAL) -> Disk DB へ書き戻し。
6.  **表示 (`dashboard.py`)**: ジャスト10分刻みで実行され、最新のDB状態を描画。
