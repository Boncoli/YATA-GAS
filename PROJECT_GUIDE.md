...
システムログはSDカードの寿命を削らないよう、以下の戦略で管理されています。
1. **日中の動作**: すべてのログ（Cronタスクのログや内部出力）は **RAMディスク (`/dev/shm/*.log`)** に直接出力されます。これによりSDカードへの細かい書き込みをゼロに抑えます。
2. **夜間の退避**: 毎朝4:35のバックアップスクリプト実行時に、RAM上の今日のログを **NAS (`/mnt/nas/yata_logs/`)** へコピー（退避）します。
3. **保持期間**: NAS上のログは過去30日分が保存され、それより古いものは自動削除されます。
> 💡 **トラブル時の調査方法**: 
> エラー調査を行う際は、**当日のログは `/dev/shm/`** を、**昨晩より前の過去のログは `/mnt/nas/yata_logs/`** を確認してください。
