...
*   **主要テーブル**:
    ...
    *   `daily_health`: [New] 1日1行の統合ヘルスケアログ（歩数、睡眠時間、睡眠詳細JSON）。
...
*   **主なタグ (Action)**:
    ...
    *   **グラフ**: `http://[RasPi_IP]:3000` (Grafana)
    ...
    ### メンテナンス記録
    - **2026/02/27 (機能拡張)**: 統合ライフログエンジン (`daily_health`) を実装。iPhoneから歩数とAutoSleepの詳細睡眠データを1日1行に集約して保存可能に。ルートディレクトリの整理と `RECOVERY_GUIDE.md` の整備を完遂。
