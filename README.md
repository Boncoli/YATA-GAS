...
3.  **Smart Error Classification (AIエラーの分別処理)**

    *   **Permanent Error**: 「コンテンツポリシー違反」や「記事が短すぎる」といったAIの拒否反応はセルに記録され、次回以降の**再処理対象から除外**されます。

    *   **Transient Error**: タイムアウトや一時的な503エラーの場合はセルを空欄のまま維持し、次回ジョブで**自動再試行**します。これにより、取りこぼしを防ぎつつ無駄なAPIコストを削減します。
...
### 7. iPhone 連携ライフログ (Mobile Integration) 【v3.6.0 New】

*   **CarPlay オートメーション**: 車の乗降（CarPlay接続/切断）をトリガーに、位置・バッテリー・メモを自動記録。
*   **インテリジェント位置補正**: 降車時と次回乗車時の距離が近い場合、GPS誤差を排除して位置情報を自動継承。
*   **自動走行距離計算 & Discord通知**: OSRM APIを使用して走行区間の道路距離を自動算出し、即座にDiscordへドライブレポートを投稿。
...
| :--- | :--- | :--- |
| **v3.6.0** | 2026-02-17 | **Intelligent Drive Log Update**<br>CarPlay連携の強化：位置情報の自動継承ロジック、OSRM APIによる走行距離の自動計算、Discordへの走行完了レポート投稿機能を実装。 |
