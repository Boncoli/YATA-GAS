...
* **`MacroTrends`**: 長期トレンド記録 (Meta Data)
...
* **`Keywords`**: 観測キーワード
* **`prompt`**: LLMプロンプト
* **`DigestHistory`**: 過去の要約履歴

### 3. デバッグ・メンテナンスコマンド

スクリプトエディタから手動実行できる関数です。

* `runAllTests()`: システム健全性の一括テスト。修正後は必ず実行。
* `testAllRssFeeds()`: 全RSSフィードの接続診断。
...
| **収集が止まる** | 特定のRSSがタイムアウトしている可能性。`RSS`シートで一時無効化 (`FALSE`)。 |
