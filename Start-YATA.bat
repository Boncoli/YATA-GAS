...
cls
echo ========================================================
echo              YATA 管理パネル [v4.5 Dashboard-Mode]
echo ========================================================

:: --- ステータス取得 (安全のため1行に変更) ---
echo | set /p="[Status] "
ssh -o ConnectTimeout=3 %SSH_USER%@%SSH_HOST% "if [ -f /dev/shm/yata.db ]; then DB=/dev/shm/yata.db; else DB=~/yata-local/yata.db; fi; echo -n 'Temp: '; vcgencmd measure_temp 2>/dev/null | cut -d= -f2 | tr -d '\n' || echo -n 'N/A'; echo -n ' | Mem: '; free -h | awk 'NR==2{print  \"/\" }' | tr -d '\n'; echo -n ' | SD: '; df -h / | awk 'NR==2{print }' | tr -d '\n'; echo -n ' | Svr: '; if pgrep -f 'yata-server' > /dev/null 2>&1; then echo -n 'ON'; else echo -n 'OFF'; fi; echo -n ' | Total: '; sqlite3 \"\" 'SELECT count(*) FROM collect;' 2>/dev/null | numfmt --grouping 2>/dev/null | tr -d '\n' || echo -n '-'; echo -n ' | ToDo: '; sqlite3 \"\" 'SELECT count(*) FROM collect WHERE summary IS NULL OR length(summary)=0;' 2>/dev/null | numfmt --grouping 2>/dev/null || echo -n '-'" || echo [接続不可]
...
if "%CHOICE%"=="20" goto SYNC_YATA_JS
...
:COLLECT
echo.
ssh %SSH_USER%@%SSH_HOST% "cd %WORK_DIR% && ./run-ram.sh do-collect.js"
pause
goto MENU

:SUMMARIZE
echo.
ssh %SSH_USER%@%SSH_HOST% "cd %WORK_DIR% && ./run-ram.sh do-summarize.js"
pause
goto MENU

:RUN_ALL
echo.
ssh %SSH_USER%@%SSH_HOST% "cd %WORK_DIR% && ./run-ram.sh yata-task.js"
pause
goto MENU
...
:LOG_VIEW
echo.
ssh -t %SSH_USER%@%SSH_HOST% "echo '=== [ 1. YATA Job Log ] ==='; tail -n 40 %WORK_DIR%/logs/yata.log; echo ''; echo '=== [ 2. Dashboard Log ] ==='; tail -n 4 %WORK_DIR%/logs/dashboard.log"
pause
goto MENU
...
ssh -t %SSH_USER%@%SSH_HOST% "if mountpoint -q /mnt/nas; then sudo image-backup /mnt/nas/rpi_complete_backup.img; else echo '[Critical Error] NAS not mounted.'; fi"
pause
goto MENU
...
echo.
echo ========================================================
echo [実行中] YATA.js 同期 (origin/main -^> lib/YATA.js)
echo ========================================================
ssh %SSH_USER%@%SSH_HOST% "cd %WORK_DIR% && git fetch origin && if ! git diff --quiet origin/main:lib/YATA.js lib/YATA.js; then git show origin/main:lib/YATA.js > lib/YATA.js; fi"
pause
goto MENU
