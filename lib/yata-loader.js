...
if (typeof AppConfig !== 'undefined') {
  try {
    ...
    /* 
    // [Experimental Patch] 一括詰め込みバッチ処理 (gpt-5.4-nano/mini 向け)
    // 💡 複数記事を1つのプロンプトにJSON配列として詰め込み、1回のAPIコールで処理する。
    // ※ nano/mini の性能やトークン制限により不安定な場合は、以下をコメントアウトして無効化してください。
      ...
      LlmService.summarizeBatch = function(articleTexts) {
        ...
        for (let i = 0; i < articleTexts.length; i += BATCH_SIZE) {
          ...
          try {
            ...
            if (!response) {
              console.log("⚠️ [Loader] LLM returned empty response for BatchDistillation.");
              throw new Error("Empty response");
            }
            console.log(`📡 [Loader] Received response (${response.length} chars). Parsing...`);
            ...
            if (parsed && parsed.results) {
              ...
              console.log("⚠️ [Loader] Parsed JSON does not contain 'results' array.");
              throw new Error("Invalid JSON structure");
            }
          } catch (e) {
            console.log(`🔄 [Loader] BatchDistillation failed (${e.message}). Falling back to original individual summarization...`);
