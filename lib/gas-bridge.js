...
process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0'; // SSL証明書エラー対策 (証明書期限切れ等のサイトも取得可能にする)
...
global.UrlFetchApp = { fetch: (url, options = {}) => {
    ...
    try {
      ...
      const text = res.text();
      return { getContentText: () => text, getResponseCode: () => res.status };
    } catch (e) {
...
const mockSheet = (name) => {
  ...
  const getTargetData = () => {
    ...
    if (n === "users") return [["名前", "メールアドレス", "配信曜日(空:毎日)", "キーワード(カンマ区切り)", "AI意味検索(TRUE/FALSE)"], ["Admin", process.env.MAIL_TO || "", "", "", "FALSE"]];
    return [[]];
  };
  ...
  return {
        ...
        if (name === "collect") {
          ...
          db.transaction((rows) => { 
            for (const r of rows) { 
              ...
              // 8番目の引数(vector)に r[6] (G列) を渡すように修正
              stmt.run(r[2], dateVal, r[1], r[2], r[3], r[4]||"", r[5]||"", r[6]||""); 
            } 
          })(values);
        }
      }
    ...
    appendRow: (row) => {
      ...
      const n = name.toLowerCase();
      if (n === "collect") {
        db.prepare('INSERT INTO collect (id, date, title, url, abstract, summary, source, vector) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING').run(p[2], p[0], p[1], p[2], p[3], p[4], p[5], "");
      }
...
global.XmlService = {
  parse: (xml) => { throw new Error("Bridge: Using Regex Fallback"); },
  getNamespace: () => ({})
};
...
global.LanguageApp = {
  translate: (text, from, to) => {
    ...
    const payloadObj = {
      model: model,
      messages: [
        { role: "system", content: "You are a professional translator. Translate the given English news title or snippet into concise, natural Japanese. Output ONLY the translated text." },
