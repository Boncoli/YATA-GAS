    ...
    getRange: (row, col, rows, cols) => ({
      ...
      setValues: (values) => {
        if (name === "collect") {
          ...
          db.transaction((rows) => { 
            for (const r of rows) { 
              if (!r[1] || !r[2] || !r[3]) continue; // URL(r[3]) が必須
              ...
              // もし summary (r[4]) が JSON 文字列（summarizeBatchの結果）ならパースして展開
              ...
              // SQLite: id, date, title, url, abstract, summary, source, vector, method_vector, tldr, who, what, when, where, why, how, result, keywords
              stmt.run(
