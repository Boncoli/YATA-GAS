  ...
  return {
        ...
        if (name === "collect") {
          ...
          db.transaction((rows) => { 
            ...
            for (let i = 0; i < rows.length; i++) {
              ...
              // 💡 [2026/03/28 物理ダンプによる最終確定構造]
              // Index 0: Date
              // Index 1: URL (ID)
              // Index 2: Link HTML
              // Index 3: Title
              // Index 4: Summary (生JSON)
              // Index 5: Source
              const id = r[1];
              const title = r[3];
              const rawSummary = r[4] || "";
              const abstract = r[2] || ""; // 本来は SiteName だが、ここではリンクHTMLを便宜上 abstract へ
              ...
              // 💡 SQLite 独自の 5W1H カラムを埋めるためのパース処理
              let tldr = r[8] || "", who = r[9] || "", what = r[10] || "", when = r[11] || "", where = r[12] || "", why = r[13] || "", how = r[14] || "", result = r[15] || "", keywords = r[16] || "";

              if (typeof rawSummary === "string" && rawSummary.startsWith("{")) {
                try {
                  const sObj = JSON.parse(rawSummary);
                  // もし配列形式（BatchDistillation結果）なら最初の要素を使う
                  const data = sObj.results ? sObj.results[0] : sObj;
                  tldr = data.tldr || tldr;
                  who = data.who || who;
                  what = data.what || what;
                  when = data.when || when;
                  ...
                  keywords = Array.isArray(data.keywords) ? data.keywords.join(", ") : (data.keywords || keywords);
                } catch (e) {}
              }

              const info = stmt.run(
                id, dateVal, title, id, abstract, rawSummary, r[5]||"", r[6]||"", r[7]||"", 
                tldr, who, what, when, where, why, how, result, keywords
              );
              if (info.changes > 0) successCount++;
            }
            console.log(`✅ [GAS-Bridge] setValues: ${successCount} rows updated, ${skipCount} rows skipped.`);
          })(values);
    ...
    appendRow: (row) => {
      ...
      if (n === "collect") {
        ...
        `);
        // 💡 1.4.1 物理ダンプによる確定インデックス: p[1] = URL, p[3] = Title
        stmt.run(
          p[1], p[0], p[3], p[1], p[2], p[4]||"", p[5]||"", p[6]||"", p[7]||"",
