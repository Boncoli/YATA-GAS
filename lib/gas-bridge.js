        ...
        if (name === "collect") {
          ...
          db.transaction((rows) => { 
            ...
            for (let i = 0; i < rows.length; i++) {
              ...
              // 💡 本家と完全に一致させるための特定ロジック
              let id = "";
              let title = "";
              let abstract = r[1] || "";
              let rawSummary = r[4] || ""; // 本家 E列 (Summary)

              if (typeof r[3] === 'string' && r[3].startsWith('http')) {
                id = r[3];
                title = r[2];
              } else if (typeof r[2] === 'string' && r[2].startsWith('http')) {
                id = r[2];
                title = r[1]; 
                abstract = ""; 
              }
              ...
              // 💡 SQLite 独自の 5W1H カラムを埋めるためのパース処理（本家の列データ rawSummary は汚さない）
