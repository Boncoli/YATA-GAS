    ...
    getRange: (row, col, rows, cols) => ({
      getValues: () => {
        ...
        if (n === "collect" && row === 2 && col === 1 && cols === 1) {
          try {
            const res = db.prepare("UPDATE collect SET vector = NULL, method_vector = NULL WHERE date < date('now', '-35 days') AND clicks = 0 AND (vector IS NOT NULL OR method_vector IS NOT NULL)").run();
