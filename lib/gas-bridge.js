...
const mockSheet = (name) => {
  ...
  return {
        ...
        if (name === "collect") {
            ...
            if (columnName) {
              ...
              db.transaction((rowsToUpdate) => {
                for (let i = 0; i < rowsToUpdate.length; i++) {
                  ...
                  // 💡 重要: YATA.js 1.2.5 では URL (ID) は index 3 に移動している
                  const id = originalRow[3] || originalRow[2]; 
          ...
          db.transaction((rows) => { 
            for (let i = 0; i < rows.length; i++) {
              const r = rows[i];
              if (!r[2] || !r[3]) continue; // Title(r[2]) と URL(r[3]) が必須
              
              const dateVal = (r[0] instanceof Date ? r[0].toISOString() : String(r[0]));
              
              // 💡 5W1Hデータのパース処理 (r[4] は summary/JSON)
              let summary = r[4] || "";
              let tldr = r[8] || "", who = r[9] || "", what = r[10] || "", when = r[11] || "", where = r[12] || "", why = r[13] || "", how = r[14] || "", result = r[15] || "", keywords = r[16] || "";

              if (typeof summary === "string" && summary.startsWith("{")) {
                try {
                  const sObj = JSON.parse(summary);
                  summary = sObj.summary || summary;
                  tldr = sObj.tldr || tldr;
                  who = sObj.who || who;
                  what = sObj.what || what;
                  when = sObj.when || when;
                  where = sObj.where || where;
                  why = sObj.why || why;
                  how = sObj.how || how;
                  result = sObj.result || result;
                  keywords = Array.isArray(sObj.keywords) ? sObj.keywords.join(", ") : (sObj.keywords || keywords);
                } catch (e) {}
              }

              // SQLite の引数の順番 (18個):
              // 1:id(r[3]), 2:date(dateVal), 3:title(r[2]), 4:url(r[3]), 5:abstract(r[1]), 6:summary(summary), 7:source(r[5]), 8:vector(r[6]), 9:method_vector(r[7]),
              // 10:tldr(tldr), 11:who(who), 12:what(what), 13:when(when), 14:where(where), 15:why(why), 16:how(how), 17:result(result), 18:keywords(keywords)
              stmt.run(
                r[3], dateVal, r[2], r[3], r[1], summary, r[5]||"", r[6]||"", r[7]||"", 
    ...
    appendRow: (row) => {
      ...
      if (n === "collect") {
        // appendRow も 17 カラム対応にする
        const stmt = db.prepare(`
          INSERT INTO collect (id, date, title, url, abstract, summary, source, vector, method_vector, tldr, who, what, "when", "where", why, how, result, keywords)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT(id) DO NOTHING
        `);
        // 💡 1.2.5 では URL は index 3
        stmt.run(
          p[3], p[0], p[2], p[3], p[1], p[4]||"", p[5]||"", p[6]||"", p[7]||"",
