        ...
        try {
          ...
          if (fs.existsSync(jsonPath)) {
              ...
              localList.forEach(item => {
                  ...
                  feedData.push([item.label, item.url, item.category || "General", isActive ? "TRUE" : "FALSE"]);
...
// --- 3. DriveApp (Local Archive) ---
global.MimeType = { PLAIN_TEXT: 'text/plain', JSON: 'application/json' };
  ...
  getFolderById: (id) => ({
    createFile: (fileName, content, mimeType) => {
      try {
        ...
        console.log(`[DriveApp(Local)] Archived to: ${filePath}`);
        return { getId: () => "local-file-id" };
      } catch (e) {
        console.error(`[DriveApp(Local)] Archive Error:`, e.message);
        throw e; // エラーを投げて呼び出し元の削除処理を中断させる
      }
    }
  })
};
