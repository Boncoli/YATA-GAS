...
/** dailyDigestJob: 日刊ダイジェスト生成 - 過去24時間の全記事（260304;キーワードフィルタリングありに修正） */
function dailyDigestJob() {
  ...
  users.forEach(user => {
    ...
    .replace("### 本日のハイライト",
            `### 本日のハイライト（KW: ${keywords.join(", ")}）`);

    
    let reportBody = LlmService.generateDailyDigest(
      systemPrompt,
      userPrompt
    );
...
function processKeywordAnalysisWithHistory(keyword, articles, options = {}) {
  ...
  // --- 進捗判定ロジック ---
  // LLMの出力に「NoChange」が含まれているか判定 (大文字小文字・スペースの揺れを許容)
  const isNoChange = /UPD:\{\s*NoChange/i.test(reportBody) || /\[NoChange\]/i.test(reportBody);
  
  let nextContext = "";
  if (isNoChange && previousSummary) {
    // 進捗がない場合は、APIの無駄打ち（コンテキスト圧縮）を防ぐため前回の履歴をそのまま引き継ぐ
    nextContext = previousSummary;
    Logger.log(`[Keep History] 「${keyword}」は進捗なし。前回の履歴をそのまま引き継ぎます。`);
  } else {
    // 2. 来週のAI用「圧縮コンテキスト」の生成 [Call 2]
    nextContext = _generateContextForNextWeek(reportBody);
  }
  ...
  // 進捗がない場合はメールレポート用本文を空(null)にして表示させない
  if (isNoChange) {
    Logger.log(`[Skip Report] 「${keyword}」は進捗がないため、メール配信から除外します。`);
    return { reportBody: null, summary: nextContext };
  }
