...
function performSemanticSearch(queryKeyword, allArticles, topN = 20) {
  ...
  for (const article of allArticles) {
    if (article.vectorStr && typeof article.vectorStr === 'string' && article.vectorStr.trim() !== "") {
...
function getArticlesInDateWindow(start, end) {
  ...
  const lastRow = sh.getLastRow();
  if (lastRow < 2) return [];
  // G列（ベクトル列）まで取得するように範囲を拡張（変更: Columns.SOURCE -> Columns.VECTOR）
  const vals = sh.getRange(2, 1, lastRow - 1, AppConfig.get().CollectSheet.Columns.VECTOR).getValues();
  const out = [];
  for (const r of vals) {
    const date = r[0];
