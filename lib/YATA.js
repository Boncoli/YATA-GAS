...
const LlmService = (function() {
  ...
  function _callOpenAiResponses(systemPrompt, userPrompt, model, apiKey, options = {}) {
    const payload = {
      ...
      max_output_tokens: options.max_output_tokens || options.max_completion_tokens || 1500,
      reasoning: {
    ...
    // ✅ コスト計算（既存ロジックに合わせる）
    if (json.usage) {
      // console.log(`[Responses API Usage] Prompt: ${json.usage.prompt_tokens}, Completion: ${json.usage.completion_tokens}`);
      _trackCost(json.usage, `OpenAI:${model}`);
    } else if (text) {
      _trackCost(systemPrompt + userPrompt, text, `OpenAI:${model}`);
    }
  ...
  return {
     ...
     */
    summarizeBatch_deprecated: function(articleTexts) {
