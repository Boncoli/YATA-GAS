    ...
    cache = {
      ...
      Llm: {
        ...
        Params: {
          ...
          ReasoningEffort: {
            NANO: props.getProperty("REASONING_NANO") || "minimal", // ローカル: none/low, 会社: minimal
          ...
          MaxCompletionTokens: {
            NANO: 2500,          // 見出し/短要約用途
            NANO_REVENGE: 4000, // 上記がlengthなどで途切れたときのリベンジ(大盛り)用
            MINI: 16000         // 分析レポート用途
          }
      ...
      System: {
        ...
        Budget: {
          ...
          RatesPer1M: {
            ...
            NANO:      { in: 0.050, out: 0.400 },     // 例: gpt-5.4-nano
            MINI:      { in: 0.250, out: 2.000 }      // 例: gpt-5.4-mini
          }
        }
      },
      // UIデザイン・メッセージ設定
...
const LlmService = (function() {
  ...
  function _trackCost(inputStr, outputStr, serviceName) {
    try {
      ...
      // 文字数を1,000,000で割り、単価を掛けてコストを算出
      const inputTokens = inputStr?.length || 0;
      const outputTokens = outputStr?.length || 0;
  ...
  function _callOpenAiLlm(systemPrompt, userPrompt, openAiModel, openAiKey, options = {}) {
    ...
    if (!isReasoningFamily) {
      ...
      // 5.4以降の推論モデルでは temperature を送らず、top_p と新パラメータで制御
      const isNano = /nano/i.test(openAiModel || "");
      payload.top_p = options.top_p ?? (isNano ? params.TopP.NANO : params.TopP.MINI);
      if (options.reasoning_effort) payload.reasoning_effort = options.reasoning_effort;
      if (options.verbosity) payload.verbosity = options.verbosity;
    }
  ...
  return {
    ...
    /**
     * summarizeBatch (安全な並列処理版)
     * 【責務】複数の記事テキストを受け取り、fetchAllで一気にAPIへ投げる。
     * 【フェイルセーフ】もし一部がエラーになっても、失敗した分だけ自動で直列フォールバックする。
     */
    summarizeBatch: function(articleTexts) {
      const targetModelType = "nano";
      const mergedOptions = _mergeDefaultLlmOptions(targetModelType, {});
      const deploymentName = llmConfig.ModelNano;
      const context = llmConfig.Context;

      const systemPrompt = getPromptConfig("BATCH_SYSTEM");
      const userTemplate = getPromptConfig("BATCH_USER_TEMPLATE");
      ...
      // 優先モデルの判定
      if (context === 'PERSONAL') {
        if (llmConfig.OpenAiKey) useOpenAi = true;
        else if (llmConfig.AzureBaseUrl && llmConfig.AzureKey) useAzure = true;
      } else {
        if (llmConfig.AzureBaseUrl && llmConfig.AzureKey) useAzure = true;
        else if (llmConfig.OpenAiKey) useOpenAi = true;
      }
      ...
      // 1. fetchAll 用のリクエスト配列を作成
      for (let i = 0; i < articleTexts.length; i++) {
        const userPrompt = userTemplate + ["", "記事: ---", articleTexts[i], "---", "", "【重要確認】上記の英語テキストから事実を抽出し、必ず「日本語（Japanese）」に翻訳してJSONで出力せよ。"].join("\n");
        let req = null;
        ...
        if (useAzure) {
          ...
          if (/json/i.test(systemPrompt) || /json/i.test(userPrompt)) {
           ...
           if (mergedOptions.temperature === 1) payload.temperature = 1;
           if (mergedOptions.reasoning_effort) payload.reasoning_effort = mergedOptions.reasoning_effort;
           if (mergedOptions.verbosity) payload.verbosity = mergedOptions.verbosity;

           req = { url: url, method: "post", contentType: "application/json", headers: { "api-key": llmConfig.AzureKey }, payload: JSON.stringify(payload), muteHttpExceptions: true };
        } else if (useOpenAi) {
           const payload = {
          ...
          if (/json/i.test(systemPrompt) || /json/i.test(userPrompt)) {
           ...
           const isReasoning = /^(gpt-5|o1|o3|o4)/.test(String(deploymentName).toLowerCase());
           if (isReasoning) {
             payload.max_completion_tokens = mergedOptions.max_completion_tokens_openai || mergedOptions.max_tokens;
             // 5.4以降は temperature ではなく top_p と新パラメータで制御
             const isNano = /nano/i.test(deploymentName || "");
             payload.top_p = mergedOptions.top_p ?? (isNano ? params.TopP.NANO : params.TopP.MINI);
           } else {
             payload.max_tokens = mergedOptions.max_tokens;
             payload.temperature = mergedOptions.temperature;
           }
           if (mergedOptions.reasoning_effort) payload.reasoning_effort = mergedOptions.reasoning_effort;
           if (mergedOptions.verbosity) payload.verbosity = mergedOptions.verbosity;

           req = { url: "https://api.openai.com/v1/chat/completions", method: "post", contentType: "application/json", headers: { "Authorization": `Bearer ${llmConfig.OpenAiKey}` }, payload: JSON.stringify(payload), muteHttpExceptions: true };
        }

        if (req) {
          requests.push(req);
          fetchMapping.push(i); // 配列の順番（座席番号）を記憶
        }
      }
      ...
      if (requests.length > 0) {
        try {
          ...
          responses.forEach((res, idx) => {
            const originalIndex = fetchMapping[idx];
            const code = res.getResponseCode();
            
            if (code === 200) {
              try {
                const apiJson = JSON.parse(res.getContentText());
                if (apiJson.choices && apiJson.choices[0]) {
                  const choice = apiJson.choices[0];
                  
                  // 🔥 ここがリベンジ判定！
                  if (choice.finish_reason === "length") {
                     Logger.log(`⚠️ Batch Index ${originalIndex} は出力枠(length)で途切れました。大盛りでリベンジします。`);
                     // results[originalIndex] は null のままにしておく（下のフォールバックへ流す）
                  } else if (choice.message && choice.message.content) {
                    const content = String(choice.message.content).trim();
                    results[originalIndex] = content;
                    _trackCost(systemPrompt + userTemplate + articleTexts[originalIndex], content, useAzure ? `Azure:${deploymentName}` : `OpenAI:${deploymentName}`);
                  }
                }
              } catch (e) {
                Logger.log("⚠️ Batch JSON Parse Error: " + e);
              }
            } else {
               Logger.log(`⚠️ Batch API Error (${code}): ${res.getContentText().substring(0, 150)}`);
            }
          });
        } catch (e) {
          Logger.log(`⚠️ Batch Network Error: ${e.toString()}`);
        }
      }

      // 3. 失敗したものは従来の直列（Geminiへのフォールバック含む）で安全に再試行
      for (let i = 0; i < results.length; i++) {
        if (!results[i]) {
          Logger.log(`🔄 Index ${i} を直列フォールバック(リベンジ🔥)で再試行します...`);
          // 💡 第二引数に true を渡すことで、AppConfigの NANO_REVENGE (2000) が適用される
          results[i] = this.summarize(articleTexts[i], true); 
        }
      }
    ...
    analyzeKeywordSearch: function(systemPrompt, contextText, options) {
        // 既存の options に taskLabel を合体させて渡す
        return _callLlmWithFallback(systemPrompt, contextText, "mini", { ...options, taskLabel: "予兆検知/インサイト分析" });
    },

    // 6. Method Vector抽出
    extractMethodDescriptor: function(title, abstractText) {
        const systemPrompt = getPromptConfig("METHOD_EXTRACTION_SYSTEM");
    ...
    if (report && report.html) {
      ...
      sendDigestEmail(
        ...
        1,
        {
          ...
          isHtml: true,
          subjectOverride: `【YATA🚨予兆検知】Emerging Signal Report (${fmtDate(new Date())})`
        }
      );
...
function dailyDigestJob() {
  ...
  users.forEach(user => {
    ...
    if (useSemantic) {
      ...
      keywords.forEach(kw => {
        const semHits = performSemanticSearch(kw, allItems, dailySearchLimit);
      ...
      filteredArticles = filterArticlesByKeywords(allItems, keywords);
    }

    if (filteredArticles.length === 0) return;

    const systemPrompt = getPromptConfig("DAILY_DIGEST_SYSTEM");
    const userPromptTemplate = getPromptConfig("DAILY_DIGEST_USER");

    const articleListText = formatArticlesForLlm(filteredArticles);
    ...
    const parsed = cleanAndParseJSON(reportBody);
    ...
    reportBody = markdownToHtml(reportBody);
...
function sendPersonalizedReport() {
  ...
  const allRecentArticles = fetchRecentArticlesBatch(FETCH_DAYS);
  ...
  users.forEach(user => {
    ...
    // レポート生成 (HTML)
    const reportHtml = generateTrendReportHtml(targetArticles, targetItems, startDate, endDate, {
      useSemantic: useSemanticForUser,
      dateRangeStr: `${fmtDate(startDate)} 〜 ${fmtDate(endDate)}` // 期間明記
    });
    ...
    try {
      // メール送信
      sendDigestEmail(null, reportHtml, null, effectiveDaysWindow, {
        recipient: email,
        isHtml: true,
...
function getVisualizationData() {
  ...
  return data
    .map(r => {
      const vector = parseVector(r[vectorColIdx]);
...
function generateTrendReportHtml(allArticles, targetItems, startDate, endDate, options = {}) {
  ...
  for (const item of targetItems) {
    ...
    if (useSemantic) {
      ...
      matched = performSemanticSearch(query, allArticles, maxResults);
    } else {
    ...
    if (result && result.reportBody) {
      ...
      if (options.isHtmlOutput) {
        // Web用
        let htmlConverted = markdownToHtml(contentBody);
...
function processKeywordAnalysisWithHistory(keyword, articles, options = {}) {
  ...
  const { reportBody } = generateWeeklyReportWithLLM(articles, [{ keyword: keyword, count: articles.length }], { [keyword]: articles }, previousSummary, options);
  if (!reportBody || reportBody.trim() === "") return null;

  const parsedJson = cleanAndParseJSON(reportBody);
  let finalMarkdown = "";
      ...
      parsedJson.topics.forEach(topic => {
        ...
        if (topic.links) {
          ...
          if (validLinks.length > 0) {
            // [[BADGE|1|https://...]] のような文字列を作っておく
            linksMd = "\n- **SOURCES:** " + validLinks.map((url, idx) => `[[BADGE|${idx + 1}|${url}]]`).join(" ");
          }
        }

        topicBlocks.push(`### ${topic.title || keyword}\n- **先週:** ${topic.last_week || "なし"}\n- **今週:** ${topic.this_week || "なし"}\n- **影響:** ${topic.impact || "なし"}${linksMd}`);
      });
  ...
  if (options.enableHistory !== false && options.saveHistory !== false) {
    nextContext = isNoChangeFlag && previousSummary ? previousSummary : _generateContextForNextWeek(finalMarkdown);
    if (nextContext) _writeHistory(keyword, nextContext);
  }
...
function getRecipients() {
  const adminMail = AppConfig.get().Digest.mailTo;
  const sheet = getSheet(AppConfig.get().SheetNames.USERS);
 ...
 * performSemanticSearch (メモリ最適化版)
 * 【責務】ベクトル類似度を使って、意味的に近い記事を検索・ソートして返す。
 */
function performSemanticSearch(queryKeyword, allArticles, topN = AppConfig.get().System.Limits.SEARCH_MAX_RESULTS, similarityThreshold = AppConfig.get().System.Thresholds.SEMANTIC_SEARCH) {
  ...
  for (const article of allArticles) {
    ...
    if (article.parsedVector) {
      const similarity = calculateCosineSimilarity(queryVector, article.parsedVector);
      ...
      const vector = parseVector(article.vectorStr);
      if (vector) {
...
function _generateAndSendDailyDigest(allArticles, config, start, end, daysWindow) {
  ...
  if (allArticles.length <= BATCH_SIZE) {
    ...
    let userPrompt = userPromptTemplate.replace(/\$\{all_articles_in_date_window\}/g, articleListText);
    const linksLimit = AppConfig.get().System.Limits.LINKS_PER_TREND || 3;
    userPrompt = userPrompt.replace(/\$\{linksPerTopic\}/g, String(linksLimit)); 
...
function processSummarization() {
  ...
  if (articlesToSummarize.length > 0) {
    ...
    for (let i = 0; i < articlesToSummarize.length; i += BATCH_SIZE) {
      ...
      const textsToEmbed = [];
      const successfulIndices = []; // バッチ結果のインデックスと紐付け
      const successfulArticles = []; // ←追加

      // 返ってきた結果を、絶対に順番がズレないようにシートデータに反映
      batchResults.forEach((jsonString, idx) => {
        const article = chunk[idx]; // 元の記事と結果を完全に紐付け
        ...
        if (newHeadline && !String(newHeadline).includes("API Error") && !String(newHeadline).includes("Safety")) {
          ...
          successfulArticles.push(article); // ←追加
      ...
      // vectorBatchResults 側で successfulArticles を使う
      if (textsToEmbed.length > 0) {
        try {
          const vectorBatchResults = LlmService.generateVectorBatch(textsToEmbed);
          ...
          vectorBatchResults.forEach((vector, i) => {
            ...
            const article = successfulArticles[i]; // ← chunk[i] ではなくこちらを使う
            ...
            if (daysOld <= vectorGenDays && METHOD_VECTOR_COL_INDEX > 0) {
              const abstractText = article.abstractText || newHeadline;
              const methodDescriptor = LlmService.extractMethodDescriptor(article.title, abstractText);
              ...
              if (methodDescriptor && !methodDescriptor.includes("[Error]")) {
                ...
                if (mVector) {
                  ...
                  values[targetRowIdx][METHOD_VECTOR_COL_INDEX] = "[Error] MethodVectorGen Failed";
                }
              } else {
                values[targetRowIdx][METHOD_VECTOR_COL_INDEX] = methodDescriptor || "[Error] Extraction Failed";
              }
            }
...
function collectRssFeeds() {
        ...
        items.forEach((item, index) => {
          ...
          try {
            ...
            if (isNaN(articleDate.getTime())) {
              articleDate = now;
            } else if (articleDate.getTime() > now.getTime() + (24 * 60 * 60 * 1000)) {
              // タイムゾーンのズレ(24時間)を超える「明らかな未来」なら今日に補正
              articleDate = now;
            }
            ...
            // 外側の allNewItems に直接貯め込む（ループ後の高速一括書き込み用）
            allNewItems.push([
              articleDate,
              cleanTitle,
              item.link,
  ...
  // ループを抜けた後に、貯めたデータを一気にシートへ書き込む（超高速）
  if (allNewItems.length > 0) {
    const lastRow = collectSheet.getLastRow();
    const maxRows = collectSheet.getMaxRows();
    const requiredRows = (lastRow + allNewItems.length) - maxRows;
    
    // 【時限爆弾の解体】シートの行数が足りない場合は、必要な分だけ事前に拡張する
    if (requiredRows > 0) {
      const buffer = AppConfig.get().System.Limits.INSERT_ROW_BUFFER || 50;
      collectSheet.insertRowsAfter(maxRows, requiredRows + buffer); // 余裕を持って追加
      SpreadsheetApp.flush(); // 行追加を確定させる
    }
...
function archiveAndPruneOldData() {
  ...
  if (validVectors.length > 0) {
    ...
    const systemPrompt = getPromptConfig("ARCHIVE_TOPIC_SYSTEM");
...
function getWebPageSummary(url) {
  try {
    ...
    // 3. LLMで要約・圧縮
    const systemPrompt = getPromptConfig("WEBPAGE_SUMMARY_SYSTEM");
...
/**
 * cleanAndParseJSON (真・完全修復版)
 * 【責務】LLMのレスポンスからJSON部分を抽出し、パースする。
 * 【自己修復】標準パース失敗時、改行の除去や正規表現による強引な抽出を行う。
 */
function cleanAndParseJSON(text) {
  ...
  // 4. 【最終手段のフォールバック】
  // 途中で途切れて } が無いような致命的エラーの場合、正規表現で強引に抜く
  try {
    // 閉じクォーテーション、または文末(?:$|")までを許容するように修正
    const match = cleaned.match(/"(?:tldr|summary)"\s*:\s*"([\s\S]*?)(?:"|$)/);
    if (match) {
      return { tldr: match[1].replace(/\n/g, " ") };
    }
  } catch (err) {}
...
function getPromptConfig(key) {
  const sheet = getSheet(AppConfig.get().SheetNames.PROMPT_CONFIG); // 
...
function getArticlesInDateWindow(start, end) {
  ...
  for (const r of vals) {
    ...
    if ((date instanceof Date) && date >= start && date < end) {
      ...
      // 共通ヘルパーで正常な見出しのみを抽出
      if (isValidHeadline(headlineStr)) {
        out.push(_createArticleObject(r));
      }
    }
  }
...
function fetchRecentArticlesBatch(maxDays) {
  ...
  return rawData.map(r => _createArticleObject(r))
                .filter(a => isValidHeadline(a.headline));
}

// --- RSS Strike System Helpers (キャッシュ最適化版) ---
 ...
 */
function filterArticlesByKeywords(allArticles, keywords) {
  ...
  return allArticles.filter(article => {
    ...
    return keywords.some(query => isTextMatchQuery(content, query));
  });
}
 ...
 */
function _createArticleObject(row) {
  const C = AppConfig.get().CollectSheet.Columns;
  let dateObj = row[0];
  ...
  return {
    ...
    parsedVector: row[C.VECTOR - 1] ? parseVector(row[C.VECTOR - 1]) : null
  };
}
...
function clearEnglishSummaries() {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (text && !text.includes("API Error") && isLikelyEnglish(text)) {
      values[i][0] = ""; // 空欄で上書き
      clearCount++;
    }
  }
...
function toolArchiveAndClearHistory() {
  const config = AppConfig.get();
  const sheet = getSheet(config.SheetNames.DIGEST_HISTORY);
...
if (typeof global !== 'undefined') {
  ...
  global.generateTrendReportHtml = generateTrendReportHtml;
