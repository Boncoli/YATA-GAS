...
const LlmService = (function() {
  ...
  return {
     ...
     * @memberof LlmService
     * @description 複数記事を並列(バッチ)で要約・抽出します。ID復元ロジックと論理リトライを備えた最新エンジン。
      ...
      for (let i = 0; i < articleTexts.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          for (let attempt = 1; attempt <= 3; attempt++) {
            try {
              response = _callLlmWithFallback(BATCH_SYSTEM, userPrompt, "nano", { 
                taskLabel: `BatchDistillation-A${attempt}`, 
                max_completion_tokens: 4000,
                reasoning_effort: "low"
              });
              // 成功判定 (JSONとしてパース可能な文字列が返ってきたか)
              if (response && (response.includes("{") || response.includes("["))) break;
              response = null;
            } catch (retryError) {
          ...
          if (!response) throw new Error("Empty or invalid response from LLM after retries");
          ...
          if (parsed && parsed.results && Array.isArray(parsed.results)) {
            Logger.log(`✅ [LlmService] Successfully parsed ${parsed.results.length} results from batch.`);
            parsed.results.forEach(res => {
              const idx = parseInt(res.id, 10);
              // IDに基づいて元の順序を復元して格納 (ID復元)
          ...
          Logger.log(`🔄 [LlmService] BatchDistillation failed (${e.message}). Falling back to individual processing...`);
          // 失敗時は1記事ずつ個別に要約（安全策）
          for (let j = 0; j < chunk.length; j++) {
            if (!results[i + j]) {
              Logger.log(`   - Falling back for article ${i + j + 1}/${articleTexts.length}`);
              results[i + j] = this.summarize(chunk[j], true);
            }
          }
        }
      }
