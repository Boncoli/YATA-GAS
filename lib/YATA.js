...
const LlmService = (function() {
  ...
  return {
    ...
    summarize: function(articleText) {
      ...
      // ★安全策: 超巨大な記事によるタイムアウトを防ぐため、1万文字でカット
      const safeText = String(articleText).substring(0, 10000);
      
      return _callLlmWithFallback(
        getPromptConfig("BATCH_SYSTEM"), 
        getPromptConfig("BATCH_USER_TEMPLATE") + ["", "記事: ---", safeText, "---"].join("\n"), 
        "nano"
      );
    },
    ...
    generateVector: function(text) {
      if (!text) return null;
      // ★安全策: Embedding APIの制限(8192 tokens)を考慮し、文字数で切り詰める
      // 10,000文字（約5000〜7000トークン）あれば、要約・見出し用途には十分かつ安全です。
      ...
      if (context === 'PERSONAL') {
        if (llmConfig.OpenAiKey) vector = _callOpenAiEmbedding(safeText, embConfig.OpenAiModel, llmConfig.OpenAiKey);
        if (!vector && embConfig.AzureEndpoint && llmConfig.AzureKey) vector = _callAzureEmbedding(safeText, embConfig.AzureEndpoint, llmConfig.AzureKey);
      } else {
        if (embConfig.AzureEndpoint && llmConfig.AzureKey) vector = _callAzureEmbedding(safeText, embConfig.AzureEndpoint, llmConfig.AzureKey);
        if (!vector && llmConfig.OpenAiKey) vector = _callOpenAiEmbedding(safeText, embConfig.OpenAiModel, llmConfig.OpenAiKey);
      }
...
function processSummarization() {
  ...
  // ★ロバスト判定: 降順前提だが、ノイズ(古い日付の混入)でbreakしないように最後までスキャンする
  for (let i = 0; i < dateValues.length; i++) {
  ...
  if (articlesToSummarize.length > 0) {
    ...
    for (const article of articlesToSummarize) {
      ...
      if (newHeadline && !String(newHeadline).includes("API Error")) {
        Logger.log(`   -> [GPT-5] ${newHeadline.substring(0, 40)}...`);
      }
...
function backfillVectors() {
  ...
  // ★ロバスト判定: 順序の揺れに備えて最後までスキャンし、範囲を特定する
  for (let i = 0; i < dateValues.length; i++) {
    const d = new Date(dateValues[i][0]);
    if (!isNaN(d.getTime()) && d >= thresholdDate) {
      targetRowCount = i + 1;
    }
  }
