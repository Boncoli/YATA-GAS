      ...
      Llm: {
        ...
        Params: {
          ...
          MaxCompletionTokens: {
            ...
            MINI: 16000     // 分析レポート用途の想定（必要に応じ調整）
          }
  ...
  return {
    ...
    summarizeBatch: function(articleTexts) {
          ...
          responses.forEach((res, idx) => {
            ...
            if (code === 200) {
              try {
                ...
                if (apiJson.choices && apiJson.choices[0] && apiJson.choices[0].message) {
                  const content = String(apiJson.choices[0].message.content).trim();
      ...
      for (let i = 0; i < results.length; i++) {
        if (!results[i]) {
          Logger.log(`🔄 Index ${i} を直列フォールバックで再試行します...`);
          results[i] = this.summarize(articleTexts[i]); // 既存の安全な関数を呼ぶ
        }
      }
    ...
    extractMethodDescriptor: function(title, abstractText) {
        const systemPrompt = "あなたはニュース記事から測定手法や技術原理を抽出する専門のAIです。\n与えられた記事タイトルと要約から、「どのような測定原理・手法・装置（Method/Modality/Assay）」が使われているかだけを抽出し、簡潔な名詞句（英語または日本語の専門用語）で答えてください。\n存在しない場合は「Unknown」と出力してください。\n出力は事実のみで、文章化せず、カンマ区切りの要素の羅列としてください。";
        const userPrompt = `Title: ${title}\nAbstract: ${abstractText}`;
        const descriptor = _callLlmWithFallback(systemPrompt, userPrompt, "nano", { temperature: 0.0 });
...
function dailyDigestJob() {
  ...
  users.forEach(user => {
    ...
    if (useSemantic) {
      keywords.forEach(kw => {
        const semHits = performSemanticSearch(kw, allItems, 10);
...
function generateTrendReportHtml(allArticles, targetItems, startDate, endDate, options = {}) {
  ...
  for (const item of targetItems) {
    ...
    if (useSemantic) {
      ...
      matched = performSemanticSearch(query, allArticles, 20);
    } else {
...
function processKeywordAnalysisWithHistory(keyword, articles, options = {}) {
  ...
  if (options.enableHistory !== false) {
    ...
    const contextForSearch = articles.map(a => a.title).join(" ").substring(0, 5000);
 ...
 * 【責務】ベクトル類似度を使って、意味的に近い記事を検索・ソートして返す。
 */
function performSemanticSearch(queryKeyword, allArticles, topN = 20, similarityThreshold = 0.32) {
...
function processSummarization() {
  ...
  const VECTOR_GENERATION_WINDOW_DAYS = 7; 
  ...
  if (articlesToSummarize.length > 0) {
    Logger.log(`${articlesToSummarize.length} 件の長文記事に対してAI要約を実行します（3件ずつ並列処理）。`);
    
    const BATCH_SIZE = 3; // 同時実行数 (安全第一で3件)
    ...
    for (let i = 0; i < articlesToSummarize.length; i += BATCH_SIZE) {
      ...
      if (textsToEmbed.length > 0) {
        try {
          ...
          vectorBatchResults.forEach((vector, i) => {
            ...
            if (daysOld <= 7 && METHOD_VECTOR_COL_INDEX > 0) {
      ...
      if (i + BATCH_SIZE < articlesToSummarize.length) {
        Utilities.sleep(2000); // 2秒休む（Azure/OpenAIのTPMリセット待ち）
      }
    }
  }
...
function backfillVectors() {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (headline && String(headline).trim() !== "") {
      ...
      if (daysOld <= 7 && METHOD_VECTOR_COL_INDEX >= 0 && (!currentMethodVector || String(currentMethodVector).trim() === "")) {
        try {
...
function collectRssFeeds() {
  ...
  if (allNewItems.length > 0) {
    ...
    if (requiredRows > 0) {
      collectSheet.insertRowsAfter(maxRows, requiredRows + 50); // 余裕を持って50行多めに追加
...
function archiveAndPruneOldData() {
  ...
  if (validVectors.length > 0) {
    ...
    // 【修正】System PromptとUser Promptを正しく分離して渡す
    const systemPrompt = "あなたはニュースのアナリストです。以下の記事タイトル群から、この期間の「主要なトレンド」を一言（30文字以内）で要約してください。";
...
function maintenanceLightenOldArticles() {
  const LIGHTEN_THRESHOLD_DAYS = 35; // 生成処理(30日)と被らないよう余裕を持たせる
...
function maintenancePruneDigestHistory() {
  const RETENTION_DAYS = 120; // 4ヶ月保存（これより古いと、話題が途切れたとみなして忘れる）
...
function maintenanceSliceVectorsTo256d() {
  ...
  const newValues = values.map((row) => {
    ...
    // 1536次元(または256次元より大きい)の場合のみスライス
    if (parts.length > 256) {
      const sliced = parts.slice(0, 256);
...
function _isRssBlacklisted(url) {
  return RssStrikeCache.get(url) >= 3;
}
...
function toolFixEnglishSummaries() {
  ...
  for (let i = 0; i < targets.length; i += BATCH_SIZE) {
    ...
    if (i + BATCH_SIZE < targets.length) {
      Utilities.sleep(3000); // APIエラー回避の3秒待機
    }
  }
