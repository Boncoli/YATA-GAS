...
const LlmService = (function() {
  ...
  function _callOpenAiResponses(systemPrompt, userPrompt, model, apiKey, options = {}) {
    const payload = {
      model: model,
      // 💡 修正: Responses APIの仕様に合わせ、システムプロンプトは instructions に指定する
    ...
    // 💡 追加: APIエラー発生時はログに出力し、空のまま処理が進むのを防ぐ
        ...
        if (item.content) {
          for (const c of item.content) {
            ...
            if ((c.type === "output_text" || !c.type) && c.text) {
              // 💡 修正: Raw JSONで c.text がオブジェクト形式（例: {value: "..."}）で返却された場合にも対応
    ...
    // ★ コスト計算 (ズレを防ぐため、usageがある時のみ正確に記録)
    if (json.usage) {
