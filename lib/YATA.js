...
function processKeywordAnalysisWithHistory(keyword, articles, options = {}) {
  ...
  let nextContext = isNoChangeFlag && previousSummary ? previousSummary : _generateContextForNextWeek(finalMarkdown);
  if (options.enableHistory !== false && options.saveHistory !== false && nextContext) _writeHistory(keyword, nextContext);
      ...
      if (textsToEmbed.length > 0) {
        try {
          ...
          vectorBatchResults.forEach((vector, i) => {
             ...
             const article = chunk[i]; // 対象の記事データ
             const newHeadline = values[targetRowIdx][SUMMARY_COL_INDEX]; // 生成済みの見出し

             // 配列長を調整（Method Vector列まで確実に拡張）
             ...
             // 2. ★追加: Method Vectorの抽出と生成 (7日以内の記事に限定)
             const articleDate = new Date(values[targetRowIdx][0]); // A列から日付取得
