...
function dailyDigestJob() {
  ...
  users.forEach(user => {
    ...
    // ▼▼▼ 追加: JSONをパースして絶対に崩れないMarkdownに組み立てる ▼▼▼
...
function processSummarization() {
  ...
  if (articlesToSummarize.length > 0) {
    ...
    for (let i = 0; i < articlesToSummarize.length; i += BATCH_SIZE) {
      ...
      // ★修正版: vectorBatchResults 側で successfulArticles を使う
  ...
  function _generateAndSetVector(rowArray, title, summary, vecColIdx) {
    try {
      ...
      if (vector) {
        ...
        // ★追加: 生成失敗時、空欄のままにせずエラー印を入れて無限リトライを防ぐ
 ...
 * 【改良】日付列だけを先にスキャンし、対象期間（直近30日）のデータのみをメモリに展開する。
 */
function backfillVectors() {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (headline && String(headline).trim() !== "") {
      ...
      // 2. ★追加: Method Vectorが欠損している場合 (7日以内の記事に限定)
...
/** * collectRssFeeds (楽観的スキップ機能付き & 一括書き込み爆速最適化版) 
...
function collectRssFeeds() {
  ...
  // ★ 変更点1: allNewItems 配列をループの外に準備（ここに全件貯める）
  let totalNewCount = 0;
  ...
  for (let i = 0; i < targetRequests.length; i += CHUNK_SIZE) {
    ...
    try {
      ...
      responses.forEach((response, idx) => {
        ...
        items.forEach((item, index) => {
          ...
          try {
            ...
            // ★ 変更点2: chunkNewItems ではなく、外側の allNewItems に直接貯め込む
            allNewItems.push([
              articleDate,
              cleanTitle,
              item.link,
      ...
      // ★ 変更点3: ここにあった激重な setValues と SpreadsheetApp.flush() を削除！
  ...
  // ★ 変更点4: ループを抜けた後に、貯めたデータを一気にシートへ書き込む（超高速）
  if (allNewItems.length > 0) {
    collectSheet.getRange(collectSheet.getLastRow() + 1, 1, allNewItems.length, allNewItems[0].length).setValues(allNewItems);
    totalNewCount = allNewItems.length;
    // ここで明示的に flush しなくても、スクリプト終了時に自動でシートに反映されます
  }
...
// ★【追加】グローバル空間にシートのキャッシュを保持（メモリ消費はごくわずかです）
...
function normalizeUrl(url) {
  ...
  // 1. クエリパラメータとアンカーを削除
  // PubMedやDOIなど、IDがパスに含まれるものはこれでOK。
  // ただし、パラメータに記事IDが含まれるタイプのサイトは注意が必要ですが、
  // 今回のリスト（論文系）はパスベースが多いため削除で安全側に倒します。
  s = s.split('?')[0].split('#')[0];
    ...
    if ((date instanceof Date) && date >= start && date < end) {
      ...
      if (headline && String(headline).trim() !== "" && String(headline).indexOf("API Error") === -1) {
        out.push({ 
          ...
          tldr: String(headline).trim(),
          vectorStr: r[6], // ★これを追加してベクトルもメモリに乗せる
          parsedVector: r[6] ? parseVector(r[6]) : null // ★【追加】読み込み時に一度だけパース
        });
      }
    }
  }
...
function fetchRecentArticlesBatch(maxDays) {
  ...
  return rawData.map(r => ({
    ...
    title: r[C.URL - 2], 
    url: r[C.URL - 1], 
    abstractText: r[C.ABSTRACT - 1], 
    headline: r[C.SUMMARY - 1], 
    source: r[C.SOURCE - 1], 
    vectorStr: r[C.VECTOR - 1],
    parsedVector: r[C.VECTOR - 1] ? parseVector(r[C.VECTOR - 1]) : null // ★【追加】
  ...
  for (let i = 0; i < targets.length; i += BATCH_SIZE) {
    ...
    batchResults.forEach((jsonString, idx) => {
      ...
      if (newHeadline && !String(newHeadline).includes("API Error") && !String(newHeadline).includes("Safety")) {
        ...
        // ★重要: 要約（意味）が変わったので、検索にヒットするようにベクトルも作り直す
