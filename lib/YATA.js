...
const LlmService = (function() {
   ...
   * @param {string} inputStr - 入力テキスト。
   * @param {string} outputStr - 出力テキスト。
   * @param {string} serviceName - 使用したモデル名（例: "Azure:gpt-5-nano"）。
   */
  ...
  return {
    ...
    /**
     * @memberof LlmService
     * @description 複数の記事を並列で要約・事実抽出します。失敗時は自動的に直列で再試行します。
     * @param {string[]} articleTexts - 要約対象のテキスト配列。
     * @returns {string[]} 要約結果（JSON文字列またはプレーンテキスト）の配列。
     */
    summarizeBatch: function(articleTexts) {
      ...
      const systemPrompt = getPromptConfig_("BATCH_SYSTEM");
      const userTemplate = getPromptConfig_("BATCH_USER_TEMPLATE");
      ...
      for (let i = 0; i < articleTexts.length; i++) {
        ...
        if (useAzure) {
          ...
          if (/json/i.test(systemPrompt) || /json/i.test(userPrompt)) {
           ...
           req = { url: url, method: "post", contentType: "application/json", headers: { "api-key": llmConfig.AzureKey, "Accept-Encoding": "gzip" }, payload: JSON.stringify(payload), muteHttpExceptions: true };
        } else if (useOpenAi) {
           const payload = {
          ...
          if (/json/i.test(systemPrompt) || /json/i.test(userPrompt)) {
           ...
           req = { url: "https://api.openai.com/v1/chat/completions", method: "post", contentType: "application/json", headers: { "Authorization": `Bearer ${llmConfig.OpenAiKey}`, "Accept-Encoding": "gzip" }, payload: JSON.stringify(payload), muteHttpExceptions: true };
        }
        ...
        if (jsonString && !String(jsonString).includes("API Error")) {
          try {
            ...
            if (parsedJson) {
              ...
              if (parsedJson.method) methodStr = String(parsedJson.method).trim(); // 🌟追加: Methodの救出
            }
...
function cleanAndParseJSON_(text) {
  ...
  try {
    ...
    const methodMatch = cleaned.match(/"method"\s*:\s*"([\s\S]*?)(?:"|$)/);
    if (methodMatch) result.method = methodMatch[1].replace(/\n/g, " ");

    if (result.tldr || result.method) return result;
  } catch (err) {}
