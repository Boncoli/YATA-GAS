      ...
      responses.forEach((response, idx) => {
        ...
        items.forEach(item => {
          try {
            ...
            // これにより、URLが変わったりタイトルが微修正された記事の重複を防ぎます。
            
            let isDuplicate = false;

            // 1. 今回のバッチ内で既に追加しようとしている記事と似ているか？
            for (const newItem of chunkNewItems) {
               const newItemTitle = decodeHtmlEntities(newItem[1]).toLowerCase();
               if (calculateLevenshteinSimilarity(normTitleToCheck, newItemTitle) > 0.9) {
                 isDuplicate = true;
                 break;
               }
            }

            // 2. 過去の収集済み記事（existingTitleSet）と似ているか？
            // ※既存3000件に対してチェックを行う
            if (!isDuplicate) {
                for (const existingTitle of existingTitleSet) {
