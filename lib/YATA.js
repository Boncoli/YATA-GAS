...
const LlmService = (function() {
  ...
  function _trackCost(inputOrUsage, outputStrOrService, serviceNameArg) {
    try {
      ...
      if (typeof inputOrUsage === 'object' && inputOrUsage !== null && (inputOrUsage.prompt_tokens !== undefined || inputOrUsage.input_tokens !== undefined)) {
...
/**
 * @description ADLM (Clinical Laboratory News) 専用のスクレイピング収集ロジック
 */
function collectAdlmFeed_() {
  const TARGET_URL = "https://myadlm.org/cln";
  const SOURCE_NAME = "Clinical Laboratory News (ADLM)";
  
  const collectSheet = getSheet_(AppConfig.get().SheetNames.TREND_DATA);
  if (!collectSheet) return;

  // 1. 既存のURLを取得して重複チェック用リストを作る（直近2000件）
  const existingUrlSet = new Set();
  const lastRow = collectSheet.getLastRow();
  
  if (lastRow >= 2) { 
    const checkLimit = AppConfig.get().System.Limits.RSS_CHECK_ROWS || 2000; 
    const numRowsToCheck = Math.min(lastRow - 1, checkLimit);
    const existingData = collectSheet.getRange(2, 3, numRowsToCheck, 1).getValues(); // C列(3)がURL
    existingData.forEach(row => {
      if (row[0]) existingUrlSet.add(normalizeUrl_(row[0])); 
    });
  }

  // 2. ADLMのページをフェッチしてスクレイピング
  const options = {
    'muteHttpExceptions': true,
    'headers': AppConfig.get().System.HttpHeaders
  };
  ...
  try {
    Logger.log(`[ADLM Scraper] ${TARGET_URL} を巡回中...`);
    const response = UrlFetchApp.fetch(TARGET_URL, options);
    
    if (response.getResponseCode() !== 200) {
      Logger.log(`[ADLM Scraper Error] サーバー応答エラー: ${response.getResponseCode()}`);
      return;
    }
    
    const html = response.getContentText();
    const regex = /<a[^>]*href="(\/cln\/articles\/[^"]+)"[^>]*>([\s\S]*?)<\/a>/gi;
    let match;
    const allNewItems = [];
    ...
    while ((match = regex.exec(html)) !== null) {
      ...
      const fullUrl = urlPath.startsWith("http") ? urlPath : "https://myadlm.org" + urlPath;
      const normalizedUrl = normalizeUrl_(fullUrl);
      
      // 文字数10文字以上のきちんとしたタイトルで、かつシートに未登録なら追加
      if (cleanTitle.length > 10 && !existingUrlSet.has(normalizedUrl)) {
        const now = new Date();
        allNewItems.push([
          now,            // A: 日付
          cleanTitle,     // B: タイトル
          fullUrl,        // C: リンクURL
          AppConfig.get().Llm.NO_ABSTRACT_TEXT, // D: 抜粋(一覧ページにはないため代替テキスト)
          "",             // E: 要約(空にして要約ジョブに任せる)
          SOURCE_NAME     // F: ソース名
        ]);
    ...
    // 3. シートの一番下に追記して、全体を日付で降順ソート
    if (allNewItems.length > 0) {
      _withRetry_(() => {
        const currentLastRow = collectSheet.getLastRow();
        collectSheet.getRange(currentLastRow + 1, 1, allNewItems.length, allNewItems[0].length).setValues(allNewItems);
        
        const newLastRow = currentLastRow + allNewItems.length;
        collectSheet.getRange(2, 1, newLastRow - 1, collectSheet.getLastColumn())
                    .sort({column: 1, ascending: false});
        SpreadsheetApp.flush(); 
      });
      Logger.log(`✅ [ADLM Scraper] ${allNewItems.length} 件の新着記事を追加しました！`);
    } else {
      Logger.log(`[ADLM Scraper] 新着記事はありませんでした。`);
    }

  } catch (e) {
    Logger.log(`❌ [ADLM Scraper Error] ${e.message}`);
  }
}
...
if (typeof global !== 'undefined') {
  ...
  global.collectAdlmFeed = collectAdlmFeed_;
