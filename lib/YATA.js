...
const LlmService = (function() {
  ...
  function _callOpenAiLlm(systemPrompt, userPrompt, openAiModel, openAiKey, options = {}) {
    const taskName = options.taskLabel ? ` / Task: ${options.taskLabel}` : "";
    Logger.log(`📡 [LLM Start] Service: OpenAI(Responses API) / Model: ${openAiModel}${taskName}`);
    ...
    // 新仕様 (Responses API) の Payload 構造
    const payload = {
      model: openAiModel,
      input: [
        { role: "system", content: systemPrompt },
    ...
    // Responses API では出力上限を max_output_tokens に統合
    payload.max_output_tokens = (options.max_completion_tokens_openai ?? options.max_tokens ?? params.MaxTokens);
    ...
    // 🌟 エンドポイントを chat/completions から responses へ変更 (Data Share無料枠適用のため)
    const json = _httpFetch("https://api.openai.com/v1/responses", fetchOptions, "OpenAI");
    
    if (json) {
      let content = null;
      // レスポンスパース (従来構造と新構造のフォールバック)
      if (json.output && Array.isArray(json.output) && json.output[0] && json.output[0].content && json.output[0].content[0]) {
        content = String(json.output[0].content[0].text).trim();
      } else if (json.choices && json.choices[0] && json.choices[0].message) {
        content = String(json.choices[0].message.content).trim();
      } else if (json.response && json.response.content) {
        content = String(json.response.content).trim();
      } else {
        Logger.log(`⚠️ [OpenAI Responses] 未知のレスポンス形式: ${JSON.stringify(json).substring(0, 200)}`);
      }

      if (content) {
        _trackCost(json.usage || { total_tokens: 0 }, `OpenAI:${openAiModel}`);
        return content;
      }
    }
    return null;
