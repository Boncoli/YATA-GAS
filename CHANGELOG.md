...
## [1.4.5-local] - 2026-03-30
### Fixed
- **GAS Bridge の挿入バグ（記事収集停止問題）の解消**:
    - v1.4.4 で導入された `setValues` の一括更新ロジックが、新規行（`lastRow + 1`）への書き込みをスキップしていた不具合を修正。
    - `columnMap` に不足していた基本カラム（date, title, url, abstract, source）を追加し、行が存在しない場合は URL を ID として `INSERT` してから `UPDATE` を行う「完全マッピング・インジェクション」を実装。
    - これにより、RSS 収集（6カラム書き込み）が正常に DB へ反映され、その後の要約ジョブへ正しくバトンが渡される環境を復旧。
